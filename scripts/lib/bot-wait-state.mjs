 ...
 * Override: JCS2_BOT_WAIT_STATE_DIR (or AR_BOT_WAIT_STATE_DIR for compatibility).
 * Default: <repo>/.jcs2-bot-wait
 */
export function botWaitStateDir(repoRoot) {
  const env =
    process.env.JCS2_BOT_WAIT_STATE_DIR?.trim() || process.env.AR_BOT_WAIT_STATE_DIR?.trim();
  const root = repoRoot || gitRepoRoot();
  if (env) {
    return path.isAbsolute(env) ? path.resolve(env) : path.resolve(root, env);
  }
  return path.join(root, '.jcs2-bot-wait');
}
...
export function legacyBotWaitStatePath(prNumber, repoRoot) {
  const root = repoRoot || gitRepoRoot();
  return path.join(root, '.git', 'jcs2-bot-wait', `${prNumber}.json`);
}

export function readBotWaitStateFile(prNumber, repoRoot) {
  const candidates = [botWaitStatePath(prNumber, repoRoot)];
  if (!process.env.JCS2_BOT_WAIT_STATE_DIR?.trim() && !process.env.AR_BOT_WAIT_STATE_DIR?.trim()) {
